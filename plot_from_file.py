...
for e in range(len(EPSILONS)):
    ...
    plt.xlabel("$FPR_{Fair}$", fontsize=22)
    plt.ylabel("$TPR_{Fair}$", fontsize=22)
    plt.xlim(0.00, 1.00)
    plt.ylim(0.00, 1.00)
    ...
    plt.savefig(
        f"figures/ROC/ROC_FROM_FILE_{DATSET_NAME}_{ALPHAS[-1]}_{EPSILONS[e]}.svg"
    )
np.savetxt(
