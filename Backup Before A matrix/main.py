...
def train_test_split(df_numeric, split=0.70):
    ...
    print(train_indexes_0.shape)
    print(train_indexes_1.shape)
    print(test_indexes_0.shape)
    print(test_indexes_1.shape)
    ...
    print(df_train_class_0.shape)
    print(df_train_class_1.shape)
    print(df_test_class_0.shape)
    print(df_test_class_1.shape)
...
def distance_raw(
    ...
    Xp = df_train[df_train[sensitive_attr[1]] == 1].drop(
        drop_columns_not_needed, axis=1
    )
    Xn = df_train[df_train[sensitive_attr[1]] == 0].drop(
        drop_columns_not_needed, axis=1
    )
    Xp_indices = Xp.index
    Xn_indices = Xn.index
...
def fast_psm(prop_diff, alpha=0.01, epsilon=0.8, n=5):
    # B = np.random.randint(-1,0,list(np.shape(prop_diff)))
    A = np.random.randint(-1, 0, list(np.shape(prop_diff)))
    prop_diff.sort(axis=1)
    sim_indices = np.where(prop_diff < alpha)
    # adjusted_xp = Xp_indices[sim_indices[0][:]]
    # adjusted_xn = Xn_indices[sim_indices[1][:]]
    # sim[adjusted_xp] = adjusted_xn

    # B[sim_indices[0][:]] = (adjusted_xp,adjusted_xn)
    dis_indices = np.where(prop_diff > epsilon)
    # adjusted_xp = Xp_indices[dis_indices[0][:]]
    # adjusted_xn = Xn_indices[dis_indices[1][:]]
...
def prepare_dataset(df_train, df_test, drop_columns_not_needed):
    ...
    print("Number of people who were rejected: ", np.argwhere(y_train == 0).shape)
    print("Number of people who were accepted: ", np.argwhere(y_train == 1).shape)
    print("Number of people who were rejected: ", np.argwhere(y_test == 0).shape)
    print("Number of people who were accepted: ", np.argwhere(y_test == 1).shape)
...
def model_performance(X_train, y_train):
    correct = 0
    total = 0
...
def consistency(pirs_sim, pairs_df, dataset_name, alpha, epsilon, n, n0, n1):
    anchors = pairs_sim.Anchor.values
    positives = pairs_sim.Positive.values
...
if __name__ == "__main__":
    alphas = [i / 1000 for i in range(1, 12, 2)]
    epsilon = 0.8
    n = 1000
    n0 = 5
    n1 = 5
    ...
    df_numeric = preprocess(df)
    print_sensitive_attr(df_numeric, drop_columns)

    df_numeric = convert_to_categorical(df_numeric, categorical_cols)
    df_numeric = df_numeric.dropna()
    df_train, df_test = train_test_split(df_numeric, split=0.70)

    drop_columns_not_needed = [
        "y"
    ]  # ["y", "propensity"] #,"propensity","duration","contact","month","day_of_week","default","pdays"]
    ...
    for alpha in alphas:
        sim, dif = balanced_psm(
            df_train, prop_diff, Xp, Xn, Xp_indices, Xn_indices, alpha, epsilon, n0, n1
        )

        # sim, dif = unbalanced_psm(
        #    df_train, prop_diff, Xp, Xn, Xp_indices, Xn_indices, n, alpha, epsilon
        # )

        pairs_sim, pairs_df = make_pairs(sim, dif, df_train)

        export(pairs_sim, pairs_df, dataset_name, alpha, epsilon, n, n0, n1)
        df_matching_pairs = make_triplets(
            pairs_sim, pairs_df, dataset_name, alpha, epsilon, n, n0, n1
        )

        export_viz_pairs(df_train, pairs_sim, dataset_name, alpha, epsilon, n, n0, n1)
        export_viz_triplets(df_matching_pairs, dataset_name, alpha, epsilon, n, n0, n1)
        ...
        ROC = consistency(pairs_sim, pairs_df, dataset_name, alpha, epsilon, n, n0, n1)
        plt.plot(ROC[:, 0], ROC[:, 1], label=alpha)
        ROC_all.append(ROC)

    # ROC_all = np.array(ROC_all)
    roc_shape = np.shape(ROC_all)
    # print(f"Shape of ROC_all is: {ROC_all.shape}")
    np.savetxt(
